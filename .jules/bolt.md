...
## 2024-05-24 - Cache Intl.NumberFormat Instances

**Learning:** Instantiating `Intl.NumberFormat` is highly expensive in JavaScript. When defined inline inside React components (e.g., as a helper function like `formatCurrency`), it gets recreated on every single render or loop iteration, creating significant performance overhead and unnecessary garbage collection.
**Action:** Always extract and cache `Intl.NumberFormat` instances globally (e.g., in a shared `utils.ts` file) outside of component render functions or loops to drastically reduce CPU overhead during formatting.
