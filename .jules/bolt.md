...
## 2024-05-16 - Hoist `new Date()` outside of list rendering loop
**Learning:** Calling `new Date()` inside a mapped render function (like `isPaymentDue` or `getDaysUntilPayment`) creates a hidden O(N) performance bottleneck when iterating through lists. For date string comparisons in 'YYYY-MM-DD' format, `new Date()` is often entirely unnecessary and can be replaced with direct string comparisons.
**Action:** When filtering or mapping over items and checking dates, hoist `const todayStr = new Date().toISOString().split('T')[0]` outside the loop or `.map()`, and perform a direct string comparison (e.g. `dateString <= todayStr`) instead of allocating objects on every loop iteration.
