...
## 2026-05-29 - O(N*M) Array.find() inside loops with complex date parsing
**Learning:** In `hooks/useSubscriptions.ts` `generateReminders()`, finding existing reminders inside a loop using `.find()` over another array, compounded by `new Date().getTime()` parsed on every iteration for both arrays, created a significant O(N*M) bottleneck during state updates.
**Action:** When matching items from two related lists (like checking if a reminder already exists for a given subscription and date), pre-calculate a `Set` of compound string keys (e.g., `id-date`) outside the loop to enable O(1) lookups and replace costly nested array `.find()` operations and redundant date parsing.
