...
## 2024-06-13 - [Reduce Chained Operations in React Hooks]
**Learning:** Found instances where `.filter().forEach()` or `.filter().reduce()` chains were used in `hooks/useSubscriptions.ts`. Benchmarking shows that iterating through a large dataset multiple times and allocating intermediate arrays takes noticeably more time. Converting `.filter().forEach()` to a single `for...of` loop, and `.filter().reduce()` to a single `.reduce()` step leads to faster execution. Additionally, `new Date(date)` instantiations inside loops are extremely slow compared to simple string matching when doing equality checks for ISO dates.
**Action:** When working on datasets in React Hooks, merge chained higher-order functions into a single pass (`.reduce()` or `for...of`), and consider strict string equality over `new Date()` when only date matching is needed.
