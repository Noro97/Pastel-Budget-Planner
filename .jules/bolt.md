...
## 2024-06-02 - [Avoid Repeated Intl.NumberFormat Instantiation in Render]

**Learning:** Instantiating `Intl.NumberFormat` or `Intl.DateTimeFormat` is an extremely slow and resource-intensive operation in JavaScript. Creating new instances of these inside component render cycles (e.g., as local `formatCurrency` or `formatDate` functions) forces the engine to recalculate formatting rules repeatedly, degrading performance especially when rendering large lists or calendar grids.
**Action:** Always extract and globally cache `Intl.NumberFormat` and `Intl.DateTimeFormat` instances in a shared utilities file (like `utils.ts`) instead of instantiating them inside React components.
