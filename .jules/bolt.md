...
## 2024-06-03 - [Formatters Bottleneck]
**Learning:** Instantiating `Intl.NumberFormat` is highly expensive and creating new instances inside component render functions or loops creates significant performance overhead.
**Action:** Extract formatting configurations (like `Intl.NumberFormat`) into shared utilities (`utils.ts`) to initialize them once and reuse the cached instance application-wide.
