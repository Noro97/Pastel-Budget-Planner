...
## 2024-05-30 - [Caching Intl.NumberFormat for O(1) Formatting]

**Learning:** Instantiating `new Intl.NumberFormat` inside functional components or calculation loops is highly expensive and creates a hidden performance bottleneck, as it creates a new instance on every render or iteration. In local micro-benchmarks, reusing a cached formatter was nearly 100x faster than creating a new instance inside a loop.
**Action:** Always create a shared, globally cached instance of `Intl.NumberFormat` (e.g., in a `utils.ts` file) and export wrapper functions. Replace any local instantiations of `new Intl.NumberFormat` with these shared functions to avoid unnecessary overhead during re-renders and list processing.
