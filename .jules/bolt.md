...
## 2024-06-08 - Cache expensive Intl formatter instances

**Learning:** Instantiating `Intl.NumberFormat` and `Intl.DateTimeFormat` objects inside React component render cycles or loops is a hidden performance bottleneck, as these constructors are computationally expensive in JavaScript.
**Action:** Always create centralized, globally cached instances of these formatters in a shared utility file (e.g., `utils.ts`) and export formatting functions to reuse the instances across the application, improving rendering performance and consistency.
