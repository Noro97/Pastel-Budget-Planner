...
## 2026-04-10 - [Global Caching of Intl.NumberFormat]
**Learning:** Instantiating `Intl.NumberFormat` locally inside React components or render loops is a hidden but significant performance bottleneck. In `SubscriptionDashboard`, `SubscriptionCalendar`, `SubscriptionList`, and `NotificationSystem`, it was instantiated on every format call, severely impacting execution speed (up to ~70x slower when generating formatting repeatedly in lists).
**Action:** Extract `Intl.NumberFormat` instances and associated formatting functions (e.g., `formatCurrency`) into a shared `utils.ts` file, globally caching the formatter. Import and reuse this cached function across components to drastically reduce initialization overhead and improve rendering speed.
