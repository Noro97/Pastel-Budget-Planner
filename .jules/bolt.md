...
## 2025-10-07 - O(N*M) Nested List Lookups and Date parsing
**Learning:** Found an O(N * M) nested lookup in `generateReminders` that scaled poorly due to performing an array `.find()` with a `new Date()` comparison inside a loop that iterated over every active subscription and their reminder days. Benchmarks indicated a 95%+ performance improvement when optimizing this.
**Action:** When finding if elements already exist between two related datasets inside loops, pre-calculate a `Set` using compound string keys (e.g., `id-date`) outside the loop to enable O(1) lookups and replace costly nested array `.find()` and `new Date()` operations.
