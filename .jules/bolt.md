...
## 2026-04-10 - [Cache Intl Formatters]
**Learning:** Instantiating `Intl.NumberFormat` or `Intl.DateTimeFormat` inside component render methods or hook closures can be a hidden performance bottleneck, because it is notoriously slow and it recreates the formatter repeatedly on every render.
**Action:** When centralizing formatter configurations, create globally cached `Intl` formatter instances in a shared utility file. Doing so ensures they are instantiated once, dramatically reducing memory allocation and improving render speeds by avoiding repeated costly setups.
