...
## 2024-05-24 - [Avoid Instantiating Intl formatters in Renders or Loops]
**Learning:** Instantiating `Intl.NumberFormat` (or `Intl.DateTimeFormat`) is highly expensive in JavaScript. Doing this inside React component render functions or inside loops (like `map` or `reduce`) creates a significant O(N) performance bottleneck.
**Action:** Always cache and reuse `Intl` formatter instances globally or outside of the execution path, e.g., in a shared utility file, to dramatically reduce CPU overhead.
