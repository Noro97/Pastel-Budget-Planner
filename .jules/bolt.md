...
## 2026-04-09 - [Cache Intl Formatters in React Components]
**Learning:** Found multiple instances where `new Intl.NumberFormat()` and `new Date().toLocaleDateString()` were being instantiated inside functional component renders (e.g., `SubscriptionDashboard`, `NotificationSystem`). Instantiating `Intl` formatters in JavaScript is a highly expensive operation. When done inside components, it creates an O(N) performance bottleneck because these instances are recreated on every re-render and inside mapping loops.
**Action:** Always cache and reuse `Intl.NumberFormat` and `Intl.DateTimeFormat` instances outside of component render functions or inside a shared utility file (like `utils.ts`) and reuse those functions.
