...
## 2024-05-25 - [O(1) Set Lookup over Array.find in Nested Loops]

**Learning:** When performing existence checks across related datasets inside loops (like checking if a reminder already exists for an active subscription), using `Array.prototype.find()` inside the loop introduces a hidden O(N*M) performance bottleneck, as the array is scanned repeatedly. Moreover, instantiating `new Date()` for date comparisons inside the `find` callback magnifies the slowdown.
**Action:** Pre-calculate the existence mapping outside the loop into a `Set` (or `Map`) keyed by compound string keys (e.g., `id-date`) to enable O(1) lookups. By converting O(N*M) lookups to O(N+M) and avoiding unnecessary date object allocations via simple string matching, the execution time was improved by ~90% for large datasets.
