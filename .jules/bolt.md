...
## 2024-05-24 - [Avoid O(N * M) Array Lookups in Loops]

**Learning:** In `hooks/useSubscriptions.ts`, checking for existing reminders inside a loop over subscriptions using `Array.find()` created an O(N \* M) bottleneck, exacerbated by repeated `new Date()` allocations inside the `.find()` callback.
**Action:** When performing existence checks across related datasets inside loops (especially React Hooks), pre-calculate a `Set` of compound string keys (e.g., `${subscriptionId}-${dueDate}`) outside the loop. This converts inner $O(M)$ lookups to $O(1)$ `Set.has()`, drastically improving performance for large data arrays.
