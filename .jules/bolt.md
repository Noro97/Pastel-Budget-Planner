...
## 2026-04-10 - [O(N*M) Lookup Optimization with Sets]

**Learning:** When checking for existence within nested loops or `.forEach` chains (e.g., verifying if a reminder already exists for each subscription), using `.find()` inside the loop creates an O(N*M) bottleneck. The overhead is severely worsened if expensive operations like `new Date()` are evaluated during every `.find()` comparison.
**Action:** Extract the lookup target into an O(1) `Set` before the loop, utilizing a compound string key (e.g., `id-date`) if comparing complex criteria. This converts the operation from O(N*M) to O(N), drastically reducing CPU overhead.
