...
## 2026-06-04 - [Cache Intl.NumberFormat for React Renders]
**Learning:** `Intl.NumberFormat` instantiation is a known expensive operation in JavaScript. Creating it inside React components or loop bodies (e.g., in `SubscriptionList.tsx`, `SubscriptionDashboard.tsx`) means it gets re-created on every render cycle and per item iterated.
**Action:** Extract formatters like `Intl.NumberFormat` to a shared utility file (e.g., `utils.ts`) outside of the React lifecycle to instantiate them only once and reuse them across the application.
