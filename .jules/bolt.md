...
## 2024-05-19 - [O(1) Set Lookups in O(N*M) Loops]

**Learning:** Checking for the existence of an item using `.find()` inside nested loops (like generating reminders for each upcoming subscription) creates an O(N\*M) execution pattern. By pre-calculating a `Set` of existing items using compound string keys (e.g. `id-date`) outside the loop, the inner lookup becomes O(1). This yielded a ~99% performance improvement on large datasets.
**Action:** When performing existance checks inside loops against another collection, pre-generate a `Set` keyed by unique identifiers before the loop to avoid redundant inner iterations.
