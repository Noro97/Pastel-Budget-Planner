...
export const useSubscriptions = (
  ...
  const generateReminders = () => {
    ...
    // ⚡ Bolt Optimization: Pre-calculate existing reminders to allow O(1) lookups
    // and replace O(N) array .find() calls inside the loop.
    // Use .getTime() to normalize date string formats to prevent strict string matching edge cases.
    const existingReminderKeys = new Set(
      reminders.map(r => `${r.subscriptionId}-${new Date(r.dueDate).getTime()}`)
    );

    // ⚡ Bolt Optimization: Use for...of instead of chaining .filter().forEach()
      ...
      for (const reminderDay of subscription.reminderDays) {
        if (daysDifference === reminderDay || daysDifference === 0) {
          ...
          if (!existingReminderKeys.has(reminderKey)) {
            ...
            // Add to Set to prevent duplicate reminders being created in the same pass
