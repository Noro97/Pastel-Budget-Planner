...
export const useSubscriptions = (
  ...
  const generateReminders = () => {
    ...
    // Optimization: Pre-calculate existing reminders into a Set for O(1) lookups
    ...
    for (let i = 0; i < subscriptions.length; i++) {
      ...
      const daysDifference = Math.ceil(
        (nextPaymentDateObj.getTime() - todayTime) / (1000 * 60 * 60 * 24)
      );
      ...
      for (let j = 0; j < subscription.reminderDays.length; j++) {
        ...
        if (daysDifference === reminderDay || daysDifference === 0) {
          const reminderKey = `${subscription.id}-${nextPaymentDate}`;
          ...
          if (!existingReminderKeys.has(reminderKey)) {
            ...
            existingReminderKeys.add(reminderKey); // Prevent duplicates within the same run
          }
        }
      }
    }
