...
const BadgeItem = ({ badge, unlocked }: { badge: Badge, unlocked: boolean }) => (
  <div className="flex flex-col items-center text-center group relative">
    <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 ${unlocked ? COLORS.status.success.bg : COLORS.neutral.bg.muted}`}>
      <badge.Icon unlocked={unlocked} />
    </div>
    <p className={`mt-2 text-sm font-semibold ${unlocked ? COLORS.neutral.text.secondary : COLORS.neutral.text.light}`}>{badge.name}</p>
...
const GamificationDashboard = ({ gamificationData }: GamificationDashboardProps) => {
  ...
  return (
    <div className={`${COMPONENTS.card} space-y-6`}>
      <h2 className={`${TYPOGRAPHY.heading.lg} ${COLORS.neutral.text.secondary}`}>Your Achievements</h2>
      ...
      <div className={`flex items-center justify-center ${COLORS.status.warning.bg} p-3 rounded-lg space-x-3`}>
